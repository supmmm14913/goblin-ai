...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.referralBonus) {
        toast.success('🎉 使用推薦碼成功！你已獲得 30 點數')
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#08080a]">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-white/30 text-sm mt-3">免費加入，立得點數</p>
        </div>
        ...
        <div className="bg-[#111114] border border-white/10 rounded-2xl p-7">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs text-white/40 mb-1.5">推薦碼（選填）</label>
              <div className="relative">
                <Gift size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/20" />
                <input type="text" className="input-field pl-9" placeholder="輸入推薦碼獲得 30 點"
          ...
          <div className="mt-4 bg-[#c8ff3e]/5 border border-[#c8ff3e]/20 rounded-xl p-3 text-center">
            <p className="text-neon text-xs font-semibold">✓ 立即獲得 10 點（使用推薦碼得 30 點）</p>
